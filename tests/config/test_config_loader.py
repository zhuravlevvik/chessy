...
from chessy.config.canonical import canonical_json, fingerprint, fingerprint_bytes
from chessy.config.loader import load_config, load_resolved
...
def test_pre_rl_resolved_config_keeps_its_stored_fingerprint(tmp_path):
    source=tmp_path/"config.yaml"
    source.write_text("""format: chessy-config-v1
name: historical
seed: 1
...
artifacts: {runs_dir: runs, dataset_manifest: null, replay_manifest: null, league_manifest: null}
""")
    _,source_bytes,resolved,_=load_config(source); payload=json.loads(resolved)
    for field in ("self_play","replay","rl","curriculum","evaluation"): payload.pop(field)
    historical_bytes=canonical_json(payload); historical=load_resolved(historical_bytes); expected=fingerprint_bytes(historical_bytes)
    run=Run.create(tmp_path,historical,source_bytes,historical_bytes,expected)
    assert Run.open(run.path).fingerprint==expected
