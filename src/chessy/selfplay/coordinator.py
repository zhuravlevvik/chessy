...
class SelfPlayCoordinator:
    ...
    def run(self, *, games: int, completed_indexes: set[int] | None = None, stop_requested: threading.Event | None = None) -> tuple[list[SelfPlayGame], list[int]]:
        ...
        def one(index:int):
            ...
            return play_game(run_id=self.run_id,run_seed=self.run_seed,generation=self.generation,game_index=index,actor_id=actor,start=start,mcts=MCTS(self.evaluator,config),schedule=self.schedule,model_checksum=self.model_checksum,stop_requested=token)
        ...
        with ThreadPoolExecutor(max_workers=self.actors,thread_name_prefix="chessy-selfplay") as pool:
            ...
            for future in as_completed(futures):
                item=future.result(); (completed.append(item) if item is not None else incomplete.append(futures[future]))
