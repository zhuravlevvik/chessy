...
def run_arena(*, candidate, opponent, positions:list, games:int, max_plies:int, candidate_checksum:str="candidate", opponent_checksum:str="opponent", config_fingerprint:str="", promotion_min_games:int=40, promotion_min_score:float=.55, confidence_threshold:float=.5, seed:int=0, progress_update:Callable[[int,int,float,str],None]|None=None) -> ArenaReport:
    ...
    for index,(start,candidate_white) in enumerate(schedule):
        ...
        if progress_update is not None: progress_update(index+1,games,value,reason)
