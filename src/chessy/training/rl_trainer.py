...
def run_rl(*,root:Path,config_path:Path|None=None,resume:Path|None=None,device:str|None=None,stop_after_steps:int|None=None)->Path:
    ...
    with StopController() as stopper:
        while state.global_step < config.scheduler.total_steps:
            ...
            if state.phase in {"initialize","selfplay"}:
                ...
                if incomplete or stopper.requested.is_set():
                    ...
                    _snapshot(run,model,optimizer,scheduler,{"format":"chessy-replay-sampler-pending-v1","draws":0},npgen,state,"stop",stopper.reason)
                    run.events.append_event("run_stopped",{"phase":state.phase}); return run.path
                old_segments=[]
                if state.replay_manifest_path:
            ...
            while state.global_step < state.training_block_end_step:
                ...
                if stopping or (state.global_step%config.training.snapshot_every_steps==0 and state.global_step<config.scheduler.total_steps):
                    _snapshot(run,model,optimizer,scheduler,sampler.state_dict(),npgen,state,"stop" if stopping else "periodic",stopper.reason or ("stop_after_steps" if stopping else None))
                if stopping:
                    run.events.append_event("run_stopped",{"phase":state.phase,"step":state.global_step}); return run.path
        ...
        run.events.append_event("run_completed",{"generation":state.generation,"step":state.global_step})
        return run.path
